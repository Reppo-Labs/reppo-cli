...
> **Status:** v0.1.0-alpha. `query balance`, `query datanet`, `query pod`, `query voting-power`, `claim-emissions`, `extend-lock`, `grant-access`, `lock`, `mint-pod`, `unlock`, and `vote` ship in alpha. The remaining 4 commands are scaffolded but not yet wired.
