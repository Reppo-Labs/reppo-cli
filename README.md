...
> **Status:** v0.1.0-alpha. `query balance`, `query pod`, `query subnet`, and `vote` ship in alpha. The remaining 11 commands are scaffolded but not yet wired.
...
- `reppo query subnet <subnetId> [--for <addr>]` — validity + REPPO access fee, optionally check access for an address
